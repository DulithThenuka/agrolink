        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {crops.map((crop) => (
            <motion.div
              key={crop.id}
              whileHover={{ y: -4 }}
              className="premium-card overflow-hidden bg-white border border-slate-100 flex flex-col justify-between h-full group"
            >
              <div className="h-48 overflow-hidden bg-slate-100 relative">
                ...
                />
                <span className="absolute top-3 left-3 bg-emerald-500 text-white text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full shadow-sm">
                  Active Listing
                </span>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">
                    ...
                    <span>{crop.location || 'Local'}</span>
                  </div>

                  <h3 className="text-base font-bold text-slate-800 mt-2 font-display hover:text-emerald-600 transition">
                    <Link to={`/crops/${crop.id}`}>{crop.name}</Link>
                  </h3>

                  <div className="mt-2.5 flex items-baseline gap-1">
                    <span className="text-xl font-extrabold text-emerald-600 font-display">${crop.price}</span>
                    <span className="text-xs text-slate-400 font-semibold">/ unit</span>
                  </div>

                  <p className="text-xs text-slate-400 font-semibold mt-1">
                    Stock available: {crop.quantity}
                  </p>
                </div>
                ...
                <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
                  {isBuyer && (
                    <button
                      ...
                      className="w-full py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-sm transition disabled:opacity-50 flex items-center justify-center gap-1.5"
                    >
